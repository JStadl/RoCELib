...
class IntervalAbstractionPytorch:
    ...
    def create_weights_and_bias_dictionary(self):
        ...
        params = {name: param.detach().numpy() for name, param in self.model.model.named_parameters()}

        weight_dict, bias_dict = {}, {}

        for layer_idx in range(len(params) // 2):
            try:
                weights = params.get(f'{layer_idx * 2}.weight')
                biases = params.get(f'{layer_idx * 2}.bias')

                if weights is None or biases is None:
                    raise KeyError(f"Unexpected parameter names in PyTorch model: layer {layer_idx}")

                for dest_idx in range(weights.shape[0]):
                    bias_dict[f'bias_into_l{layer_idx + 1}_n{dest_idx}'] = [biases[dest_idx] - self.bias_delta,
                                                                            biases[dest_idx] + self.bias_delta]

                    for src_idx in range(weights.shape[1]):
                        weight_dict[f'weight_l{layer_idx}_n{src_idx}_to_l{layer_idx + 1}_n{dest_idx}'] = [
                            weights[dest_idx, src_idx] - self.delta, weights[dest_idx, src_idx] + self.delta]
